...
function initWebSocket() {
    ...
    ws.onopen = () => {
        console.log('Connected to VulnSight X engine');
        ...
        <table class="w-full">
            <thead class="bg-gray-800">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left">CVSS Score</th>
            ...
            <tbody>
                ${vulnerabilities.map(v => `
                    <tr class="border-b border-gray-700 hover:bg-gray-800">
                        <td class="px-4 py-2">
                            <span class="px-2 py-1 rounded text-xs font-semibold vuln-${v.severity.toLowerCase()}">
                                ${v.severity}
                            </span>
                        </td>
                        ...
                        <td class="px-4 py-2 text-sm">
                            <details>
                                ...
                                <div class="mt-2 text-xs">
                                    <p><strong>Payload:</strong> ${v.payload || 'N/A'}</p>
...
function updateDashboard() {
    ...
    const avgScore = vulnerabilities.reduce((sum, v) => sum + parseFloat(v.cvssScore || 0), 0) / vulnerabilities.length;
...
function displayResponse(response) {
    ...
    if (response.success) {
        contentDiv.innerHTML = `
            ...
            <div class="mb-2">
                <span class="font-bold">Time:</span> ${response.timeMs}ms
            </div>
            <div class="mb-2">
                <span class="font-bold">Size:</span> ${response.size} bytes
            </div>
            <div class="mt-4">
                <span class="font-bold">Response Body:</span>
                <pre class="mt-2 p-2 bg-gray-900 rounded overflow-x-auto">${escapeHtml(JSON.stringify(response.body, null, 2))}</pre>
            </div>
        `;
    } else {
        contentDiv.innerHTML = `
...
<html>
<head>
    <title>VulnSight X Security Report</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 40px; background: #f5f5f5; }
        .container { max-width: 1200px; margin: 0 auto; background: white; padding: 30px; border-radius: 10px; }
        h1 { color: #333; }
        .vuln { margin: 20px 0; padding: 15px; border-left: 4px solid; border-radius: 4px; }
        .critical { border-color: #ef4444; background: #fee; }
        .high { border-color: #f59e0b; background: #fff3e0; }
        .medium { border-color: #eab308; background: #fef9e3; }
        .low { border-color: #10b981; background: #e0f2e9; }
...
<body>
    <div class="container">
        <h1>VulnSight X Security Assessment Report</h1>
        ...
        <table>
            <thead>
                <tr><th>Severity</th><th>Type</th><th>Endpoint</th><th>CVSS Score</th></tr>
            </thead>
            <tbody>
                ${vulnerabilities.map(v => `
                    <tr>
        ...
        <ul>
            ${vulnerabilities.map(v => `<li>${v.type}: Implement proper input validation and output encoding</li>`).join('')}
        </ul>
    </div>
</body>
</html>
...
function toggleViews(viewId) {
    const views = ['dashboardView', 'scannerView', 'interceptorView'];
    views.forEach(id => {
